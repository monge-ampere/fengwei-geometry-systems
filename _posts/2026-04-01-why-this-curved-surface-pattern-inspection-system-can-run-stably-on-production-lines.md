...
这一点我后来在交接材料里其实已经说得很直接：算法由于使用 `C++` 编写，在框架设计时采用了并行处理，线程间临界区已经做到最小，因此算法层面的进一步提速难度已经很大；而现有上位机却仍然采用串行处理，即先 `PLC` 通信，再采集图片，再做算法处理，最后显示结果，后一个过程必须等前一个过程结束才能执行。这样一来，在真实检测过程中，系统经常会在生产间隙里出现“等”的现象，也就是说，整套系统的吞吐并没有把这些间隙时间真正利用起来。
